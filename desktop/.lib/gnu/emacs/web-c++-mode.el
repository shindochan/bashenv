;; Web-C++ code editing commands for Emacs
;; 1987 Dave Detlefs  (dld@cs.cmu.edu) 
;; and  Stewart Clamen (clamen@cs.cmu.edu).
;; Done by fairly faithful modification of:
;; c-mode.el, Copyright (C) 1985 Richard M. Stallman.
;;
;; Fri Feb 20 11:52:12 PST 1998 (Mick O'Halloran, mick@tera.com
;;   Added require c++-mode, because it does, the syntax table for to
;;   copy. 
;; Thu Nov 20 13:50:36 PST 1997 (Mick O'Halloran, mick@tera.com)
;;   Changed the key binding of web-c++-indent-defun to \C-c\C-q, which
;;   is where the C version goes.
;; Oct, 1989 (Dave Detlefs, dld@cs.cmu.edu)
;;   Added contribution from Igor Metz <metz@iam.unibe.ch>:
;;     functions web-c++-comment-region and web-c++-uncomment-region and 
...
;;   Changed calculate-web-c++-indent to handle member initializations
;;   more flexibly. Two new variables are used to control behavior:
;;   web-c++-member-init-indent and web-c++-continued-member-init-offset.
...
;;   Fixed calculate-web-c++-indent to handle continued lines ending in
...
;;   Dan Weinreb (dlw@odi.com) pointed out that 'web-c++-mode successively
;;   bound web-c++-indent-exp and web-c++-indent-defun to ESC-^q.  ESC-^q is
;;   now bound to web-c++-indent-exp, while, web-c++-indent-defun is invoked
;;   with ESC-^x.

;; February 1989 (Dave Detlefs, dld@cs.cmu.edu)
;;   Fixed some errors in web-c++-indent-defun, as pointed out by Sam
...
(require 'cc-mode)
(defvar web-c++-mode-abbrev-table nil
  "Abbrev table in use in Web-C++-mode buffers.")
(define-abbrev-table 'web-c++-mode-abbrev-table ())

(defvar web-c++-mode-map ()
  "Keymap used in Web-C++ mode.")
(if web-c++-mode-map
    ()
  (setq web-c++-mode-map (make-sparse-keymap))
  (define-key web-c++-mode-map "\C-j" 'reindent-then-newline-and-indent)
  (define-key web-c++-mode-map "{" 'electric-web-c++-brace)
  (define-key web-c++-mode-map "}" 'electric-web-c++-brace)
  (define-key web-c++-mode-map ";" 'electric-web-c++-semi)
  (define-key web-c++-mode-map "\e\C-h" 'mark-c-function)
  (define-key web-c++-mode-map "\e\C-q" 'indent-web-c++-exp)
  (define-key web-c++-mode-map "\177" 'backward-delete-char-untabify)
  (define-key web-c++-mode-map "\t" 'web-c++-indent-command)
  (define-key web-c++-mode-map "\C-c\C-i" 'web-c++-insert-header)
  (define-key web-c++-mode-map "\C-c\C-\\" 'web-c++-macroize-region)
  (define-key web-c++-mode-map "\C-c\C-c" 'web-c++-comment-region)
  (define-key web-c++-mode-map "\C-c\C-u" 'web-c++-uncomment-region)
  (define-key web-c++-mode-map "\e\C-a" 'web-c++-beginning-of-defun)
  (define-key web-c++-mode-map "\e\C-e" 'web-c++-end-of-defun)
  (define-key web-c++-mode-map "\C-c\C-q" 'web-c++-indent-defun))

(defvar web-c++-mode-syntax-table nil
  "Syntax table in use in Web-C++-mode buffers.")

(if web-c++-mode-syntax-table
    ()
  (setq web-c++-mode-syntax-table (copy-syntax-table c-mode-syntax-table))
  (modify-syntax-entry ?/ ". 12" web-c++-mode-syntax-table)
  (modify-syntax-entry ?\n ">" web-c++-mode-syntax-table)
  (modify-syntax-entry ?\' "." web-c++-mode-syntax-table))

(defvar web-c++-continued-member-init-offset nil
...
(defvar web-c++-member-init-indent 0
  "*Indentation level of member initializations in function declarations.")
(defvar web-c++-friend-offset -4
  "*Offset of Web-C++ friend class declarations relative to member declarations.")
(defvar web-c++-electric-colon t
  "*If t, colon is an electric terminator.")
(defvar web-c++-empty-arglist-indent nil
  "*Indicates how far to indent an line following an empty argument
list.  Nil indicates to just after the paren.")


(defun web-c++-mode ()
  "Major mode for editing Web-C++ code.  Very much like editing C code.
Expression and list commands understand all Web-C++ brackets.
Tab at left margin indents for Web-C++ code
...
Variables controlling indentation style:
 ...
 web-c++-electric-colon
    If non-nil at invocation of web-c++-mode (t is the default) colon electricly
    indents.
 web-c++-empty-arglist-indent
 ...
 web-c++-friend-offset
    Offset of Web-C++ friend class declarations relative to member declarations.
 web-c++-member-init-indent
    Indentation level of member initializations in function declarations,
    if they are on a separate line beginning with a colon.
 web-c++-continued-member-init-offset
...
Settings for K&R, BSD, and Stroustrup indentation styles are
  ...
  web-c++-empty-arglist-indent                4
  web-c++-friend-offset                       0

Turning on Web-C++ mode calls the value of the variable web-c++-mode-hook with
no args,if that value is non-nil."
  (interactive)
  (kill-all-local-variables)
  (use-local-map web-c++-mode-map)
  (setq major-mode 'web-c++-mode)
  (setq mode-name "Web-C++")
  (setq local-abbrev-table web-c++-mode-abbrev-table)
  (set-syntax-table web-c++-mode-syntax-table)
  ...
  (setq indent-line-function 'web-c++-indent-line)
  ...
  (setq comment-indent-hook 'web-c++-comment-indent)
  (make-local-variable 'parse-sexp-ignore-comments)
  (setq parse-sexp-ignore-comments nil)
  (run-hooks 'web-c++-mode-hook)
  (if web-c++-electric-colon
      (define-key web-c++-mode-map ":" 'electric-web-c++-terminator)))

;; This is used by indent-for-comment
;; to decide how much to indent a comment in Web-C++ code
;; based on its context.
(defun web-c++-comment-indent ()
  (if (looking-at "^\\(/\\*\\|//\\)")
...
(defun electric-web-c++-brace (arg)
  ...
  (let (insertpos)
    (if (and (not arg)
	     ...
	     (or (save-excursion
		   (skip-chars-backward " \t")
		   (bolp))
		 (if c-auto-newline (progn (web-c++-indent-line) (newline) t) nil)))
	(progn
...
(defun electric-web-c++-semi (arg)
  "Insert character and correct line's indentation."
  (interactive "P")
  (if c-auto-newline
      (electric-web-c++-terminator arg)
    (self-insert-command (prefix-numeric-value arg))))

(defun electric-web-c++-terminator (arg)
  "Insert character and correct line's indentation."
  (interactive "P")
...
(defun web-c++-indent-command (&optional whole-exp)
  "Indent current line as Web-C++ code, or in some cases insert a tab character.
...
The relative indentation among the lines of the expression are preserved."
  ...
  (if whole-exp
      ...
      (let ((shift-amt (web-c++-indent-line))
	    beg end)
	(save-excursion
...
(defun web-c++-indent-line ()
  "Indent current line as Web-C++ code.
Return the amount the indentation changed by."
  (let ((indent (calculate-web-c++-indent nil))
	beg shift-amt
	  ...
	  (t
	   ...
	   (cond ((looking-at "\\(default\\|public\\|private\\|protected\\):")
		 ...
		 ((looking-at "friend\[ \t]class[ \t]")
		  (setq indent (+ indent web-c++-friend-offset)))
		 ((= (following-char) ?})
...
(defun calculate-web-c++-indent (&optional parse-start)
  "Return appropriate indentation for current line as Web-C++ code.
      ...
      (cond ((or (nth 3 state) (nth 4 state))
	    ...
	    ((null containing-sexp)
		 ...
		 0   ; Unless it starts a function body
	       (web-c++-backward-to-noncomment (or parse-start (point-min)))
	       (if (= (preceding-char) ?\))
		 ...
		 (if (= (preceding-char) ?})
		   ...
		   (skip-chars-forward " \t")
		   (if (= (following-char) ?:)
		       (if web-c++-continued-member-init-offset
			   (+ (current-indentation)
			      web-c++-continued-member-init-offset)
			 (progn
			   (forward-char 1)
	    ...
	    ((/= (char-after containing-sexp) ?{)
	     ...
	     ;; web-c++-empty-arglist-indent says to do.
	     (if (and web-c++-empty-arglist-indent
		 ...
		 (progn
		   ...
		   (beginning-of-line)
		   (skip-chars-forward " \t")
		   (goto-char (min (+ (point) web-c++-empty-arglist-indent)
				   (1+ containing-sexp)))
		   (current-column))
	    ...
	    (t
	     ...
	     (goto-char indent-point)
	     (web-c++-backward-to-noncomment containing-sexp)
...
(defun web-c++-backward-to-noncomment (lim)
  (let (opoint stop atcode (cpoint (point)))
    (while (not stop)
      (skip-chars-backward " \t\n\r\f" lim)
      (setq opoint (point))
...
(defun indent-web-c++-exp ()
  "Indent each line of the Web-C++ grouping following point."
  (interactive)
	  ...
	  (if (eolp)
		...
		(if (/= (char-after (car contain-stack)) ?{)
		  ...
		  (save-excursion
		    ...
		    (web-c++-backward-to-noncomment opoint)
		    (if (not (memq (preceding-char) '(nil ?\, ?\; ?} ?: ?{)))
	      ...
	      (let ((val (calculate-web-c++-indent
			  (if (car indent-stack)
			      (- (car indent-stack))))))
		(setcar indent-stack
		...
		(setq this-indent (+ this-indent web-c++-friend-offset)))
	    (if (= (following-char) ?})
		(setq this-indent (- this-indent c-indent-level)))
	    (if (= (following-char) ?{)
...
(defun web-c++-insert-header ()
  "Insert header denoting Web-C++ code at top of buffer."
  (interactive)
  (save-excursion
    (goto-char (point-min))
    (insert "// "
...
;;; This page covers "macroization;" making Web-C++ parameterized types
;;; via macros.

(defvar web-c++-default-macroize-column 78
  "Place to insert backslashes.")

(defun web-c++-macroize-region (from to arg)
...
(defun backslashify-current-line (doit)
  ...
  (cond
   (doit
	...
	(progn
	  (if (>= (current-column) web-c++-default-macroize-column)
	      (insert " \\")
	    (while (<= (current-column) web-c++-default-macroize-column)
	      (insert "\t") (end-of-line))
	    (delete-char -1)
	    (while (< (current-column) web-c++-default-macroize-column)
	      (insert " ") (end-of-line))
...
;;; grammar of Web-C++; the weak one just figures anything id or curly on
;;; the left begins a defun.  The constant "web-c++-match-header-strongly"
;;; determines which to use; the default is the weak one.

(defvar web-c++-match-header-strongly nil
  "*If NIL, use web-c++-defun-header-weak to identify beginning of definitions,\
if nonNIL, use web-c++-defun-header-strong")

(defvar web-c++-defun-header-strong-struct-equivs "\\(class\\|struct\\|enum\\)"
  "Regexp to match names of structure declaration blocks in Web-C++")

(defconst web-c++-defun-header-strong
  (let*
      (; valid identifiers
	 ...
	 (concat "\\(:"
		 ...
		 "\\|" wh-opt "{\\)"))
       (type-name (concat
		    "\\(" web-c++-defun-header-strong-struct-equivs wh-nec "\\)?"
		    id))
	 ...
	 (concat modifiers type wh-nec func-name wh-opt dcl-list wh-opt inits))
       (inherit (concat "\\(:" wh-opt "\\(public\\|private\\)?"
			wh-nec id "\\)"))
       (cs-header (concat
		    web-c++-defun-header-strong-struct-equivs
...
(defconst web-c++-defun-header-weak "^{\\|^[_a-zA-Z].*{"
  "Weakly-defined regexp to match beginning of structure \
or function definition.  ")


(defun web-c++-beginning-of-defun (arg)
  (interactive "p")
  (let ((web-c++-defun-header (if web-c++-match-header-strongly
			      web-c++-defun-header-strong
			    web-c++-defun-header-weak)))
    (cond ((or (= arg 0) (and (> arg 0) (bobp))) nil)
	  ((and (not (looking-at web-c++-defun-header))
		(let ((curr-pos (point))
		      (open-pos (if (search-forward "{" nil 'move)
				    (point)))
		      (beg-pos
			(if (re-search-backward web-c++-defun-header nil 'move)
			    (match-beginning 0))))
		  (if (and open-pos beg-pos
		    ...
		    nil))))
	  (t
	    (if (and (looking-at web-c++-defun-header) (not (bobp)))
		(forward-char (if (< arg 0) 1 -1)))
	    (and (re-search-backward web-c++-defun-header nil 'move (or arg 1))
		 (goto-char (match-beginning 0)))))))


(defun web-c++-end-of-defun (arg)
  (interactive "p")
  (let ((web-c++-defun-header (if web-c++-match-header-strongly
			      web-c++-defun-header-strong
			    web-c++-defun-header-weak)))
    (if (and (eobp) (> arg 0))
	nil
      (if (and (> arg 0) (looking-at web-c++-defun-header)) (forward-char 1))
      (let ((pos (point)))
	(web-c++-beginning-of-defun 
	  (if (< arg 0)
	      (- (- arg (if (eobp) 0 1)))
	    arg))
	(if (and (< arg 0) (bobp))
	    t
	  (if (re-search-forward web-c++-defun-header nil 'move)
	      (progn (forward-char -1)
		     (forward-sexp)
		     (beginning-of-line 2)))
	  (if (and (= pos (point)) 
		   (re-search-forward web-c++-defun-header nil 'move))
	      (web-c++-end-of-defun 1))))
      t
      ))
  )

(defun web-c++-indent-defun ()
